...
public class ProductoRepositoryAdapter implements ProductoRepositoryPort {
	...
	public Producto crear(Producto producto) {
		ProductoEntity busqueda = jpaRepository.findByNombre(producto.getNombre());
		if (busqueda != null) {
			throw new NegocioException("APIPROD001", "PRODUCTO YA EXISTE",
					"NO PUEDE CREAR PORQUE YA EXISTE UN PRODUCTO IGUAL");
		}
	...
	public Producto getProductoById(Long id) {
		return jpaRepository.findById(id)
				.map(e -> new Producto(e.getId(), e.getNombre(), e.getDescripcion(), e.getPrecio())).orElse(null);
	}

	@Override
	public List<Producto> listar() {
		return jpaRepository.findAll().stream()
				.map(e -> new Producto(e.getId(), e.getNombre(), e.getDescripcion(), e.getPrecio()))
